// Package cache stores resolved lyrics in a local SQLite database.
//
// A cached row is an optimisation, never an authority: every hit is re-matched
// against the caller's query before it is served, so a fuzzy /api/search
// result can never become a confident /api/get answer by sitting in the cache.
...
//
// Identity here is deliberately NOT matching.NormalizeText: that function is
...
// matching.IdentityKey so that the dispatcher's deduplication and this storage
...
func GenerateCacheKey(title, artist string, duration float64) string {
	sum := sha256.Sum256([]byte(matching.IdentityKey(title, artist, duration)))
...
func Matches(q model.Query, item *model.LyricResponse) bool {
	...
	return matching.MatchTrack(q, item.TrackName, item.ArtistName, item.AlbumName, item.Duration) != matching.MatchNone
}
